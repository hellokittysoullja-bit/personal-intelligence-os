...
## Milestone 1 — Foundation (реализовано)

### Фактический результат

Реализовано ровно по плану ниже, с двумя мелкими отличиями от
первоначального предложения (оба — детали реализации, не архитектурные
решения):

- `apps/web/app/page.tsx` вместо `apps/web/src/app/page.tsx` — Next.js App
  Router по умолчанию ищет `app/` в корне пакета, не в `src/`; сохранено
  соглашение фреймворка вместо `src/app`.
- В `@pios/observability` `createLogger` получил необязательный параметр
  `destination` (переопределение потока вывода) — понадобился для
  надёжного unit-теста редактирования секретов (pino пишет напрямую в fd
  через sonic-boom в обход `process.stdout.write`, что делает
  `vi.spyOn(process.stdout, "write")` ненадёжным). Не влияет на поведение
  в production (по умолчанию — обычный stdout).

Дерево, зависимости и критерии готовности ниже соответствуют фактически
реализованному состоянию репозитория.
